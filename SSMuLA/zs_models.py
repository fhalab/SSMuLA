...
class ProtBert(ZeroShotPrediction):
    def __init__(self, seq_path):
        super().__init__(seq_path)
        self.tokenizer = BertTokenizer.from_pretrained(
            "Rostlab/prot_bert_bfd", do_lower_case=False
        )
        self.model = BertForMaskedLM.from_pretrained("Rostlab/prot_bert_bfd")
        self.alphabet_size = len(self.tokenizer.vocab)
        self.unmasker = pipeline(
            "fill-mask",
            model=self.model,
            tokenizer=self.tokenizer,
            top_k=self.alphabet_size,
        )
        self.n_sites = 4
        self.mask_string, self.cls_string, self.eos_string = (
    ...
    def get_mut_pos(self, mut):
        """Get the position of the mutation in the sequence."""
        return int(mut[1:-1])

    @staticmethod
    def mask_sequence(sequence: str, mask_position: int, mask_string: str) -> str:
        """
        Mask a sequence at given positions and return the masked sequence 
        and the positions of the masked residues.
        """
        separator = " "
        sequence = separator.join(sequence)
        masked_sequence = (
            sequence[:mask_position] + mask_string + sequence[mask_position + 1 :]
        )
        return masked_sequence

    @staticmethod
    def get_score_with_key(scores: List[dict], key: str, newAA: str) -> Optional[float]:
        """
        Retrieve the 'score' value from a list of dictionaries 
        where the specified key matches the newAA.
        """
        return next((dic["score"] for dic in scores if dic.get(key) == newAA), None)

    def run_protBert(self, wt_seq):
        variants = self.read_seq().iloc[:, 0]  # Get variants from csv file
        score = np.zeros((len(variants), self.n_sites))  # Scores
        ...
        for i, variant in enumerate(variants):
            ...
            for j, mut in enumerate(muts):
                wt, number, newAA = self.split_input(mut)
                # Mask the sequence
                masked_seq = self.mask_sequence(
                    wt_seq, (number - 1) * 2, self.mask_string
                )
                scores = self.unmasker(
                    masked_seq
                )  # Get probability for each vocab item
                prob = self.get_score_with_key(scores, "token_str", newAA)
                score[i, j] = prob
        return score